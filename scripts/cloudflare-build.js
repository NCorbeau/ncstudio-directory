...
function getChangedFiles() {
      ...
      try {
        ...
        try {
          ...
          try {
            ...
            changedFiles = execSync('git show --name-only --pretty="" HEAD', {
              encoding: "utf8",
            })
              .split("\n")
              .filter((file) => file.trim() !== "");
            ...
            console.log(
              "Could not determine changed files, will build all directories"
            );
            return [];
          }
        }
  ...
  console.log(
    "No changed files information available, building all directories"
  );
  return [];
}
...
function getDirectoriesToBuild(changedFiles) {
  ...
  changedFiles.forEach((file) => {
    ...
    if (
      file.startsWith("src/components/core/") ||
...
async function buildAllDirectories() {
  ...
  const directories = fs.readdirSync(BUILD_DIR).filter((dir) => {
  ...
  // Copy API functions to the correct location
  copyApiFunctions();
...
/**
 * Copy API functions to the correct location for Cloudflare Pages
 * Functions should be directly in dist/api/ not in dist/functions/api/
 */
function copyApiFunctions() {
  console.log(
    "Copying API functions to correct location for Cloudflare Pages..."
  );
  ...
  if (!fs.existsSync(functionsApiDir)) {
    console.warn("No API functions found in functions/api directory");
    return;
  }

  // Create the api directory in the output if it doesn't exist
  if (!fs.existsSync(outputApiDir)) {
    fs.mkdirSync(outputApiDir, { recursive: true });
  }

  // Copy the API function files
  const entries = fs.readdirSync(functionsApiDir, { withFileTypes: true });

  for (const entry of entries) {
    const srcPath = path.join(functionsApiDir, entry.name);
    const destPath = path.join(outputApiDir, entry.name);

    if (entry.isDirectory()) {
      // If it's a subdirectory, copy it recursively
      copyDir(srcPath, destPath);
    } else {
      // Copy the file
      fs.copyFileSync(srcPath, destPath);
      console.log(`✓ Copied ${entry.name} to dist/api/`);
    }
  }

  console.log("API functions copied successfully to dist/api/ directory");
}
...
function createDirectorySelector() {
  ...
  if (fs.existsSync(directoriesPath)) {
    ...
    const distFolders = fs.readdirSync(BUILD_DIR).filter((folder) => {
...
async function runBuild() {
  ...
  if (isFirstBuild) {
    console.log(
      "🚀 First deployment detected - performing full build of all directories..."
    );
    await buildAllDirectories();

    // Create a marker file to indicate first build is done
    fs.writeFileSync(
      path.join(BUILD_DIR, "first-build-completed"),
